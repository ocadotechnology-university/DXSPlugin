...
var paths = require('./cjs/paths-Cebs5I6U.cjs.js');
...
const _DefaultCacheClient = class _DefaultCacheClient {
  constructor(client, clientFactory, options) {
    ...
    __privateAdd$2(this, _clientFactory, void 0);
    __privateAdd$2(this, _options, void 0);
    __privateSet$1(this, _client, client);
    __privateSet$1(this, _clientFactory, clientFactory);
    __privateSet$1(this, _options, options);
  }
  async get(key) {
    const k = this.getNormalizedKey(key);
    const value = await __privateGet$1(this, _client).get(k);
    return value;
  }
  async set(key, value, opts = {}) {
    const k = this.getNormalizedKey(key);
    await __privateGet$1(this, _client).set(k, value, opts.ttl);
  }
  async delete(key) {
    const k = this.getNormalizedKey(key);
    await __privateGet$1(this, _client).delete(k);
  }
  withOptions(options) {
    const newOptions = { ...__privateGet$1(this, _options), ...options };
    return new _DefaultCacheClient(
      __privateGet$1(this, _clientFactory).call(this, newOptions),
      __privateGet$1(this, _clientFactory),
      newOptions
    );
  }
  /**
...
var _getJwtExpiration$1, getJwtExpiration_fn$1;
...
class DefaultAuthService {
  constructor(tokenManager, identity, pluginId, disableDefaultAuthPolicy) {
    ...
    __privateAdd$1(this, _getJwtExpiration$1);
  }
  ...
  async authenticate(token) {
    ...
    return createCredentialsWithUserPrincipal(
      ...
      token,
      __privateMethod$1(this, _getJwtExpiration$1, getJwtExpiration_fn$1).call(this, token)
    );
  }
  ...
  async getLimitedUserToken(credentials) {
    ...
    return { token, expiresAt: __privateMethod$1(this, _getJwtExpiration$1, getJwtExpiration_fn$1).call(this, token) };
  }
}
_getJwtExpiration$1 = new WeakSet();
getJwtExpiration_fn$1 = function(token) {
...
var _getJwtExpiration, getJwtExpiration_fn, _auth, _extractCredentialsFromRequest, extractCredentialsFromRequest_fn, _getCredentials, getCredentials_fn;
class AuthCompat {
  constructor(identity, tokenManager) {
    this.identity = identity;
    this.tokenManager = tokenManager;
    __privateAdd(this, _getJwtExpiration);
  }
  isPrincipal(credentials, type) {
    const principal = credentials.principal;
    if (principal.type !== type) {
  ...
  async authenticate(token) {
    ...
    if ((payload == null ? void 0 : payload.aud) === "backstage") {
      const identity = await this.identity.getIdentity({
        request: {
          headers: { authorization: `Bearer ${token}` }
        }
      ...
      return createCredentialsWithUserPrincipal(
        identity.identity.userEntityRef,
        token,
        __privateMethod(this, _getJwtExpiration, getJwtExpiration_fn).call(this, token)
      );
    }
    await ((_a = this.tokenManager) == null ? void 0 : _a.authenticate(token));
  ...
  async getPluginRequestToken(options) {
    ...
    switch (type) {
      case "service": {
        ...
        return { token: (_a = internalForward.token) != null ? _a : "" };
      }
      case "user":
        if (!internalForward.token) {
          throw new Error("User credentials is unexpectedly missing token");
  ...
  async getLimitedUserToken(credentials) {
    ...
    return { token, expiresAt: __privateMethod(this, _getJwtExpiration, getJwtExpiration_fn).call(this, token) };
  }
}
...
class HttpAuthCompat {
  constructor(auth) {
    __privateAdd(this, _extractCredentialsFromRequest);
    __privateAdd(this, _getCredentials);
    __privateAdd(this, _auth, void 0);
    __privateSet(this, _auth, auth);
  }
  async credentials(req, options) {
    const credentials = await __privateMethod(this, _getCredentials, getCredentials_fn).call(this, req);
    const allowed = options == null ? void 0 : options.allow;
    if (!allowed) {
      return credentials;
    }
    if (__privateGet(this, _auth).isPrincipal(credentials, "none")) {
      if (allowed.includes("none")) {
        return credentials;
      }
      throw new errors.AuthenticationError("Missing credentials");
    } else if (__privateGet(this, _auth).isPrincipal(credentials, "user")) {
    ...
    } else if (__privateGet(this, _auth).isPrincipal(credentials, "service")) {
...
extractCredentialsFromRequest_fn = async function(req) {
  ...
  if (!token) {
    return __privateGet(this, _auth).getNoneCredentials();
  }
  return __privateGet(this, _auth).authenticate(token);
};
_getCredentials = new WeakSet();
getCredentials_fn = async function(req) {
  var _a;
  return (_a = req[credentialsSymbol]) != null ? _a : req[credentialsSymbol] = __privateMethod(this, _extractCredentialsFromRequest, extractCredentialsFromRequest_fn).call(this, req);
};
...
function createLegacyAuthAdapters(options) {
  ...
  const identity = (_a = options.identity) != null ? _a : pluginAuthNode.DefaultIdentityClient.create({ discovery });
